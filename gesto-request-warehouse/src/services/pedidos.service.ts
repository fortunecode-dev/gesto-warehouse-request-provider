...
export const syncProducts = async (url: string, productos: any[]) => {
  try {
    const areaId = await AsyncStorage.getItem('selectedLocal');
    const response = await axios.post(`${API_URL}/request/sync/${url}`, { productos, areaId });
...
export const activateRequest = async () => {
  try {
    ...
    const response = await axios.post(`${API_URL}/request/send-to-warehouse/${areaId}`);
    return response.data;
  } catch (error) {
    return []
  }
...
export const makeMovement = async () => {
  try {
    ...
    const response = await axios.post(`${API_URL}/request/make-movement/${areaId}`);
    return response.data;
  } catch (error) {
    return []
  }
